...
export function TacticalBrief({
  ...
  const modalRef = useModalFocus({ active: true, initialFocus: "#brief-title", onDismiss: onClose });
    ...
    <div ref={modalRef} className="ledger-overlay" role="dialog" aria-modal="true" aria-labelledby="brief-title" tabIndex={-1}>
      <section className="forge-ledger forge-ledger--brief">
        <header className="forge-ledger__header">
          ...
          <span>
            <small>争焰 · 开局设置</small>
            <h2 id="brief-title" tabIndex={-1}>选择执火者与规则</h2>
          </span>
          <button type="button" onClick={onClose} aria-label={selectionLocked ? "关闭角色与规则" : "返回游戏大厅"}>×</button>
        </header>
